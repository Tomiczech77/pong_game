...
RYCHLOST_PALKY = RYCHLOST * 1.5 # v pixelech za sekundu

DELKA_PULICI_CARY = 20
VELIKOST_FONTU = 42
ODSAZENI_TEXTU = 30

pozice_palek = [VYSKA // 2, VYSKA // 2 ] # vertikalni pozice dvou palek
pozice_mice = [0, 0] # x, y souradnice micku -- nastaveno v reset()
rychlost_mice = [0, 0] # x, y slozky micku -- nastaveno v reset()
...
def vykresli():
    ...
    obrazek = pyglet.image.load("background.jpg")
    pozadi = pyglet.sprite.Sprite(obrazek)
    pozadi.scale = 2
    pozadi.draw()
    ...
    for y in range(DELKA_PULICI_CARY // 2, VYSKA, DELKA_PULICI_CARY * 2):
        nakresli_obdelnik(
            SIRKA // 2 - 1,
            y,
            SIRKA // 2 + 1,
...
window.push_handlers(
    ...
    on_key_press=stisk_klavesy, # po stisknuti klaves pohyb palek (vstup uživatele)
    on_key_release=pusteni_klavesy, # po puštění kláves konec pohybu (vstup uživatele)
)
